...
def run_lens_valuation(
    ...
    try:
        ...
        if events:
            pw = ProbabilityWeightEngine.from_config_list(events)
            weighted_cap = pw.apply(sotp_cap_hkd)
...
def main():
    ...
    if selected == "603993" and val.get("segments"):
        ...
        st.markdown('<p class="section-header">🏭 AI铜需求逻辑</p>', unsafe_allow_html=True)
        st.markdown("""
**报告来源：** 《AI基建大周期下铜的投资价值与全球化投资策略研究报告》
        
**核心逻辑：**
- AI数据中心每兆瓦(MW)装机铜消耗量高达 **27-33吨**
- BloombergNEF预测到2035年数据中心内部铜累计消耗将超过 **430万吨**
- AI算力集群变配电重构+电网扩容，每GW需铜约 **5,500吨**
- 高盛估计至2030年AI电网投资将达 **7,200亿美元**
- 铜矿供给面临结构性约束：新矿开发周期长达16年、全球品位下降40%、TC/RC深度负值
- 2025年全球铜精矿供需缺口已达 **84.8万吨** 且在持续扩大
        
**对洛阳钼业的启示：**
- CMOC刚果金TFM+KFM是全球成本曲线前25%的优质铜矿资产
- 铜钴业务贡献86%利润，AI驱动的铜需求结构性增长直接利好
- 在铜价$9000-10000区间，公司净利润有望维持200-300亿量级
        
**风险：**
- 刚果金地缘政治风险（15%概率触发，影响-35%）
